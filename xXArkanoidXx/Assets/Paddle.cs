...
public class Paddle : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((body.position.x + (paddleWidth / 2) > 3.2 && hAxis > 0) || (body.position.x - (paddleWidth / 2) < -3.2 && hAxis < 0))
        {
        ...
        if (currentlyHeldPowerUp != null)
        {
            switch (currentlyHeldPowerUp)
            {
                case PowerUp.PowerUpType.Laser:
                    ...
                    if (Input.GetKeyDown(KeyCode.Space))
                    {
                        GameObject laser0 = Instantiate(laserShot, this.transform.position.WithX(transform.position.x - 0.2f), Quaternion.identity);
                        laser0.GetComponent<Rigidbody2D>().velocity = new Vector2(0f, 15f);
                        GameObject laser1 = Instantiate(laserShot, this.transform.position.WithX(transform.position.x + 0.2f), Quaternion.identity);
                        laser1.GetComponent<Rigidbody2D>().velocity = new Vector2(0f, 15f);
                        if (--laserAmmo < 1)
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Ball"))
        {
            ...
            if (currentlyHeldPowerUp == PowerUp.PowerUpType.Stick && ballSticks == false)
            {
                ...
                collision.gameObject.transform.position = ownPosition + new Vector3(0f, 0.2f, 0f);
        ...
        else if (collision.gameObject.CompareTag("PowerUp"))
        {
            ...
            if (currentlyHeldPowerUp == PowerUp.PowerUpType.Stick && effect == PowerUp.PowerUpType.Stick)
            {
            ...
            else
            {
                ...
                if (effect == PowerUp.PowerUpType.Laser)
                    laserAmmo = 3;
            }
        }
    }
